...
//   }
  ...
  */
const stringLength = stg => stg.length
console.log(stringLength('Edilson'));  
  /*
    03
  ...
  */
  const stgLowerCase = stg => stg.toLowerCase()
  console.log(stgLowerCase('EDILSON Fer'));
  /*
    04
  ...
  */
  const caracterPosition =(caracter, stg) => stg.indexOf(caracter)

  console.log(caracterPosition('s','Edilson'));
  /*
    05
  ...
  */
  const isIncludes =(item,array) =>  array.includes(item)
  
  const nomes = ['João','Pedro','Daniel']
  console.log(isIncludes('Daniel',nomes));
  ...
  */
  ...
  /*
    ...
    - Invoque a função que recebe um callback por parâmetro, passando como  
      ...
      foi exibido.
  */
  const myName = () => console.log('Edilson CallBack');
  ...
  */
  const tiple = value => value*3
  const call = (func) => func(33)

  console.log(call(tiple));
  ...
  */
  ...
  numbers.forEach((element,index,array) =>console.log(`O ${index+1}º item do array [${array.join(', ')}] é ${element}`))
...
console.log(lettersCopy);
  ...
  */
 nomesPessoas =['Edilson','Daniel','Maria','Pedro']
 function postLikes(likesPersons) {
    retorno = ''
    arrayLength =likesPersons.length
    if(arrayLength ==0){
        retorno ='Ninguém curtiu isso'
    }else if(arrayLength ==1){
        retorno =`${likesPersons[0]} curtiu isso`
    }else if(arrayLength ==2){
        retorno =`${likesPersons[0]} e ${likesPersons[1]} curtiram isso`
    }else if(arrayLength ==3){
        retorno =`${likesPersons[0]}, ${likesPersons[1]} e ${likesPersons[2]} curtiram isso`
    } else {
         retorno =`${likesPersons[0]}, ${likesPersons[1]} e mais ${arrayLength-2} curtiram isso`
    }
    return retorno
 }
  console.log(postLikes(nomesPessoas));